    ...
    for dial_dict in dials:
        ...
        for ti, turn in enumerate(dial_dict["turns"]):
            ...
            if turn_domain not in EXPERIMENT_DOMAINS:
                continue
            if data_type == 'train':
                turn_uttr = (turn['system_transcript'] + ' ; ' + turn['user_transcript']).strip()
            else:
                turn_uttr = (turn['system_transcript'] + ' ; ' + turn['user_asr']).strip()
            ...
            if multi_granularity:
                ...
                instance = create_instance(dialog_history, state_history, size_window, n_history, tokenizer, ti,
                                           len_turns, dialogue_id, turn_domain, turn_id, turn_dialog_state,
                                           slot_meta, max_seq_length, op_code)
                data.append(instance)

    ...
    for dial_dict in dials:
        ...
        for ti, turn in enumerate(dial_dict["turns"]):
            ...
            if 'system_utterance' not in turn:
                turn_uttr = '[SEP] ' + turn['user_utterance']['text'].strip()
            else:
                turn_uttr = (turn['system_utterance']['text'] + ' [SEP] ' + turn['user_utterance']['text']).strip()
            ...
            if multi_granularity:
                ...
                size_window = min(size_window, len(dialog_history))
                instance = create_instance(dialog_history, state_history, size_window, tokenizer, ti,
                                           len_turns, dialogue_id, turn_domain, turn_id, turn_dialog_state,
                                           slot_meta, max_seq_length)
                data.append(instance)

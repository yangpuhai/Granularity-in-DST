...
OP = {'other': 0, 'none': 1, 'dontcare': 2, 'yes': 3, 'no': 4}
...
def make_span(slot, turn_utter, lang, value):
    result = fix_value_span(slot, lang, value, turn_utter)
    return result
    ...
    for slot in slot_meta:
        ...
        if turn_value != window_value:
            if turn_value == 'dontcare':
                ...
                generate_y.append(make_span(slot, window_utter, lang, turn_value))
        else:
            op_labels.append('none')
            generate_y.append([0, 0])

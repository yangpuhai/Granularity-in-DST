...
SLOT=['restaurant-price','restaurant-people','restaurant-name','restaurant-category','restaurant-location','restaurant-date','restaurant-time','restaurant-meal','restaurant-rating']
...
def create_instance(dialog_history, state_history, size_window, n_history, tokenizer, ti, len_turns, dialogue_id,
                    turn_domain, turn_id, turn_dialog_state, slot_meta, max_seq_length, op_code):
    ...
    if len(dialog_history) > size_window and n_history > 0:
        history_uttr = dialog_history[-(size_window + n_history):-size_window]
    ...
    op_labels, generate_y, gold_state = make_turn_label(slot_meta, window_dialog_state,
                                                        turn_dialog_state,
                                                        tokenizer, op_code)

    instance = TrainingInstance(dialogue_id,turn_domain,
                                turn_id, " ; ".join(dialog_history[-size_window:]), history_uttr,
                                window_dialog_state, op_labels, generate_y, gold_state, max_seq_length, slot_meta,
                                is_last_turn, op_code=op_code)
    instance.make_instance(tokenizer)
    return instance

def prepare_dataset(data_scale, data_path, tokenizer, slot_meta, size_window, n_history, max_seq_length,
                    op_code='4', multi_granularity = False, data_type = ''):
    ...
    for dial_dict in dials:
        dial_dict["domains"] = EXPERIMENT_DOMAINS
        ...
        for ti, turn in enumerate(dial_dict["turns"]):
            ...
            if 'system_utterance' not in turn:
                turn_uttr = turn['user_utterance']['text'].strip()
            else:
                turn_uttr = (turn['system_utterance']['text'] + ' ; ' + turn['user_utterance']['text']).strip()
            ...
            if multi_granularity:
                ...
                for sw in range(1, max_size_window + 1):
                    instance = create_instance(dialog_history, state_history, sw, n_history, tokenizer, ti,
                                               len_turns, dialogue_id, turn_domain, turn_id, turn_dialog_state,
                                               slot_meta, max_seq_length, op_code)
                    data.append(instance)
            else:
                size_window1 = min(size_window, len(dialog_history))
                instance = create_instance(dialog_history, state_history, size_window1, n_history, tokenizer, ti,
                                           len_turns, dialogue_id, turn_domain, turn_id, turn_dialog_state,
                                           slot_meta, max_seq_length, op_code)
                data.append(instance)

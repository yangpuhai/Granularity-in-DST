...
def load_data(train_dials, lang, slot_meta, size_window, MG=False, data_type = ''):
    ...
    for dial_dict in train_dials:
        ...
        for ti, turn in enumerate(dial_dict["turns"]):
            ...
            if data_type == 'train':
                user_uttr = turn['user_transcript'].strip()
            else:
                user_uttr = turn['user_asr'].strip()
            ...
            turn_dialog_state = {}
            for s in turn['state']:
                turn_dialog_state[s] = turn['state'][s]
...
def prepare_dataset(train_scale, random_seed, train_data_path, dev_data_path, test_data_path, slot_meta, train_size_window=0, train_MG=False, test_size_window=0, test_MG=False):
    ...
    for dial_dict in train_dials:
        for turn in dial_dict["turns"]:
            ...
            user_tokens = turn['user_transcript'].strip().split(' ')
            lang.index_words(user_tokens,'list')
            user_tokens_asr = turn['user_asr'].strip().split(' ')
            lang.index_words(user_tokens_asr,'list')
            for s in turn['state']:
                lang.index_words(s.split(" "), 'list')
                lang.index_words(turn['state'][s].split(" "), 'list')

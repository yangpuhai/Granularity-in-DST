...
class DataProcessor(object):
    ...
    def get_train_examples(self, data_dir, data_scale):
...
class Processor(DataProcessor):
    ...
    def __init__(self, config):
        ...
        # DSTC2 dataset
        if config.data_dir == "data/DSTC2" or config.data_dir=="data/DSTC2-turn":
            fp_ontology = open(os.path.join(config.data_dir, "original/ontology_dstc2.json"), "r")
        ...
        # simR dataset
        elif config.data_dir == "data/M2M/sim-R" or config.data_dir=="data/simR-turn":
            fp_ontology = open(os.path.join(config.data_dir, "original/ontology.json"), "r")
            ontology = json.load(fp_ontology)
            for slot in ontology.keys():
                ontology[slot].append("do not care")
                ontology[slot].append("none")
        ...
        # simM dataset
        elif config.data_dir == "data/M2M/sim-M" or config.data_dir=="data/simM-turn":
            fp_ontology = open(os.path.join(config.data_dir, "original/ontology.json"), "r")
            ontology = json.load(fp_ontology)
            for slot in ontology.keys():
                ontology[slot].append("do not care")
        ...
        # WOZ2.0 dataset
        elif config.data_dir == "data/woz" or config.data_dir=="data/woz-turn":
        ...
        # MultiWOZ dataset
        elif config.data_dir == "data/multiwoz2.1":
            fp_ontology = open(os.path.join(config.data_dir, "original/ontology.json"), "r")
            ontology_original = json.load(fp_ontology)
            ontology = {}
            for domain_slot in ontology_original.keys():
                ...
                if slot == "leaveAt" and domain != "bus":
                    ...
                    slot = "price range"
                slot = domain + '-' + slot
                ontology[slot] = ontology_original[domain_slot]
                if 'dontcare' in ontology[slot]:
                    dontcare_idx = ontology[slot].index('dontcare')
                    ontology[slot][dontcare_idx] = 'do not care'
                ontology[slot].append("none")
            fp_ontology.close()
    ...
    def get_train_examples(self, data_dir, data_scale, accumulation=False):
        """See base class."""
        return self._create_examples_scale(
            self._read_tsv(os.path.join(data_dir, "train.tsv")), "train", data_scale, accumulation)
    ...
    def _create_examples_scale(self, lines, set_type, data_scale, accumulation=False):
        ...
        random.seed(42)
        dialogue_ids = random.sample(dialogue_ids, int(data_scale*len(dialogue_ids)))
        random.seed()

        prev_dialogue_index = None
        examples = []
        for (i, line) in enumerate(lines):
            if line[0] not in dialogue_ids:
                continue
            guid = "%s-%s-%s" % (set_type, line[0], line[1])  # line[0]: dialogue index, line[1]: turn index
            if accumulation:
                if prev_dialogue_index is None or prev_dialogue_index != line[0]:
...
def convert_train_examples_to_features(examples, label_list, max_seq_length, tokenizer, max_turn_length, train_size_window, batch_size, n_gpu):
    ...
    features = []
    for train_size_window1 in range(1,train_size_window+1):
        features.append([])
    ...
    all_padding_len = [0, 0] * train_size_window
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        dialogue_history.append([tokens_a, tokens_b])
        label_id = []
        label_info = 'label: '
        ...
        label_history.append(label_id)

        curr_dialogue_idx = example.guid.split('-')[1]
        if curr_dialogue_idx not in dialogue_ids:
            dialogue_ids.append(curr_dialogue_idx)
        curr_turn_idx = int(example.guid.split('-')[2])

        for train_size_window1 in range(1,train_size_window+1):
            features[train_size_window1-1] = create_input(features[train_size_window1-1], dialogue_history, label_history, max_seq_length, tokenizer, max_turn_length, train_size_window1, \
                                            all_padding, all_padding_len, slot_dim, prev_dialogue_idx, prev_turn_idx, curr_dialogue_idx, curr_turn_idx, example, ex_index)

        prev_dialogue_idx = curr_dialogue_idx
        prev_turn_idx = curr_turn_idx

    for train_size_window1 in range(1,train_size_window+1):
        features[train_size_window1-1] = create_input2(features[train_size_window1-1], dialogue_history, label_history, max_seq_length, tokenizer, max_turn_length, train_size_window1, \
                                        all_padding, all_padding_len, slot_dim, prev_dialogue_idx, prev_turn_idx, example, ex_index)
    
    
    # # multi_gpu setting
    # if n_gpu > 1:
    #     if (len(dialogue_ids) % batch_size) % 2 == 1:
    #         features += [InputFeatures(input_ids=all_padding,
    #                                     input_len=all_padding_len,
    #                                     label_id=[-1]*slot_dim)]*max_turn_length
    
    features = [f1 for f in features for f1 in f]
    ...
    all_input_len = all_input_len.view(-1, max_turn_length, 2*train_size_window)
    all_label_ids = all_label_ids.view(-1, max_turn_length, slot_dim)

    return all_input_ids, all_input_len, all_label_ids

def create_input(features, dialogue_history, label_history, max_seq_length, tokenizer, max_turn_length, train_size_window1, all_padding, all_padding_len,\
    slot_dim, prev_dialogue_idx, prev_turn_idx, curr_dialogue_idx, curr_turn_idx, example, ex_index):

    if prev_dialogue_idx is not None and prev_dialogue_idx != curr_dialogue_idx:
        lost_dialogue_len = (prev_turn_idx + 1) % train_size_window1
        if lost_dialogue_len != 0:
            lost_label = label_history[-(lost_dialogue_len + 1)]
            lost_dialogues = [d for dia in dialogue_history[-(lost_dialogue_len + 1): -1] for d in dia]
            max_len1 = max_seq_length - (sum([1 if dia is not None else 0 for dia in lost_dialogues]) + 1)
            _truncate_multi_seq_pair(lost_dialogues, max_len1)
            tokens = []
            input_len = deepcopy(all_padding_len)
            for di in range(len(lost_dialogues)):
                if lost_dialogues[di] is not None:
                    input_len[di] = len(lost_dialogues[di])+1
                    tokens += lost_dialogues[di] + ["[SEP]"]
                else:
                    input_len[di] = 0
                if di == 0:
                    input_len[di] += 1
            ...
            features.append(
                InputFeatures(input_ids=input_ids,
                              ...
                              label_id=lost_label))
            if ex_index < 4:
                logger.info("*** Last Example ***")
                logger.info("guid: %s" % example.guid)
                logger.info("tokens: %s" % " ".join(
                    [str(x) for x in tokens]))
                logger.info("input_ids: %s" % " ".join([str(x) for x in input_ids]))
                logger.info("input_len: %s" % " ".join([str(x) for x in input_len]))
                logger.info("label: " + str(lost_label))
            
        turn_number = math.ceil((prev_turn_idx + 1) / train_size_window1)
        if turn_number <= max_turn_length:
            features += [InputFeatures(input_ids=all_padding,
                                        input_len=all_padding_len,
                                        label_id=[-1]*slot_dim)]\
                        *(max_turn_length - turn_number)
        assert len(features) % max_turn_length == 0
    end_of_window = (curr_turn_idx + 1) % train_size_window1 is 0
    if end_of_window:
        turn_number = (curr_turn_idx + 1) // train_size_window1
        if prev_dialogue_idx is None or turn_number <= max_turn_length:
            label_id = label_history[-1]
            dialogues = [d for dia in dialogue_history[-train_size_window1:] for d in dia]
            max_len1 = max_seq_length - (sum([1 if dia is not None else 0 for dia in dialogues]) + 1)
            _truncate_multi_seq_pair(dialogues, max_len1)
            tokens = []
            input_len = deepcopy(all_padding_len)
            for di in range(len(dialogues)):
                if dialogues[di] is not None:
                    input_len[di] = len(dialogues[di])+1
                    tokens += dialogues[di] + ["[SEP]"]
                if di == 0:
                    input_len[di] += 1
                    tokens = ["[CLS]"] + tokens
            input_ids = tokenizer.convert_tokens_to_ids(tokens)
            ...
            if ex_index < 4:
                ...
                logger.info("input_ids: %s" % " ".join([str(x) for x in input_ids]))
                logger.info("input_len: %s" % " ".join([str(x) for x in input_len]))
                logger.info("label: " + str(label_id))
    return features

def create_input2(features, dialogue_history, label_history, max_seq_length, tokenizer, max_turn_length, train_size_window1, all_padding, all_padding_len,\
    slot_dim, prev_dialogue_idx, prev_turn_idx, example, ex_index):
        lost_dialogue_len = (prev_turn_idx + 1) % train_size_window1
        if lost_dialogue_len != 0:
            lost_label = label_history[-(lost_dialogue_len + 1)]
            lost_dialogues = [d for dia in dialogue_history[-(lost_dialogue_len + 1): -1] for d in dia]
            ...
            for di in range(len(lost_dialogues)):
                if lost_dialogues[di] is not None:
                    input_len[di] = len(lost_dialogues[di])+1
                    input_ids += lost_dialogues[di] + ["[SEP]"]
                if di == 0:
                    input_len[di] += 1
...
def _truncate_multi_seq_pair(tokens_list, max_length):
    ...
    # total_length1 = sum([len(tokens) if tokens is not None else 0 for tokens in tokens_list])
    while True:
        total_length = sum([len(tokens) if tokens is not None else 0 for tokens in tokens_list])
        if total_length <= max_length:
            break
        max_len = 0
        max_id = 0
        for i in range(len(tokens_list)):
            if tokens_list[i] is not None:
                if len(tokens_list[i]) > max_len:
                    max_len = len(tokens_list[i])
                    max_id = i
        tokens_list[max_id].pop()


def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, max_turn_length, batch_size, n_gpu):
    """Loads a data file into a list of `InputBatch`s."""
...
def main():
    ...
    parser.add_argument('--train_size_window',
                        type=int,
                        default=1,
                        help="dialogue state update window size in MGL")
    ...
    if args.do_train:
        train_examples = processor.get_train_examples(args.data_dir, args.train_scale, accumulation=accumulation)
        dev_examples = processor.get_dev_examples(args.data_dir, accumulation=accumulation)

        ## Training utterances
        all_input_ids, all_input_len, all_label_ids = convert_train_examples_to_features(
            train_examples, label_list, args.max_seq_length, tokenizer, args.max_turn_length, args.train_size_window, args.train_batch_size, n_gpu)
        ...
        logger.info("  num_train_batches = %d", num_train_batches)
        ...
        all_input_ids_dev, all_input_len_dev, all_label_ids_dev = convert_examples_to_features(
            dev_examples, label_list, args.max_seq_length, tokenizer, args.max_turn_length, args.dev_batch_size, n_gpu)

        logger.info("***** Running validation *****")
        logger.info("  Num examples = %d", len(dev_examples))
        logger.info("  Num batchs = %d", all_input_ids_dev.size(0))
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        all_input_ids, all_input_len, all_label_ids = convert_examples_to_features(
            eval_examples, label_list, args.max_seq_length, tokenizer, args.max_turn_length, args.eval_batch_size, n_gpu)
        all_input_ids, all_input_len, all_label_ids = all_input_ids.to(device), all_input_len.to(device), all_label_ids.to(device)
        logger.info("***** Running evaluation *****")
        logger.info("  Num examples = %d", len(eval_examples))
        logger.info("  Num batchs = %d", all_input_ids.size(0))
        ...
        for input_ids, input_len, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            accuracies = eval_all_accs(args.data_dir, pred_slot, label_ids, accuracies)
...
def eval_all_accs(data_dir, pred_slot, labels, accuracies):
    ...
    def _eval_acc(_pred_slot, _labels):
        ...
        joint_acc = torch.sum(torch.sum(accuracy, 1) == slot_dim, 0).float()
        # slot accuracy

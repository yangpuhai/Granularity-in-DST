...
SLOT=['price_range','num_people','restaurant_name','category','location','date','time','meal','rating']
...
def load_data(train_dials, lang, slot_meta, size_window, MG=False):
    # load training data
    data = []
    for dial_dict in train_dials:
        ...
        for ti, turn in enumerate(dial_dict["turns"]):
            ...
            if 'system_utterance' in turn:
                ...
                system_uttr = system_utterance['text'].strip()
            if 'user_utterance' in turn:
                user_utterance = turn['user_utterance']
                user_uttr = user_utterance['text'].strip()
            if 'system_utterance' in turn:
            ...
            turn_dialog_state = {}
            for s in turn['dialogue_state']:
                turn_dialog_state[s['slot']] = s['value']
...
def prepare_dataset(train_scale, random_seed, train_data_path, dev_data_path, test_data_path, slot_meta, train_size_window=0, train_MG=False, test_size_window=0, test_MG=False):
    ...
    for dial_dict in train_dials:
        for turn in dial_dict["turns"]:
            if 'system_utterance' in turn:
                system_utterance = turn['system_utterance']
                system_tokens = system_utterance['tokens']
                lang.index_words(system_tokens,'list')
            if 'user_utterance' in turn:
                user_utterance = turn['user_utterance']
                user_tokens = user_utterance['tokens']
                lang.index_words(user_tokens,'list')
            for s in turn['dialogue_state']:
                lang.index_words(s["slot"].split("_"), 'list')
                lang.index_words(s["value"].split(" "), 'list')

    #load training data
    train_data = load_data(train_dials, lang, slot_meta, train_size_window, train_MG)
    dev_data = load_data(dev_dials, lang, slot_meta, test_size_window, test_MG)
    test_data = load_data(test_dials, lang, slot_meta, test_size_window, test_MG)

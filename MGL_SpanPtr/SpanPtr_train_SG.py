...
from SpanPtr_evaluation import model_evaluation, model_evaluation_batch
...
def main(args):
    ...
    if args.dataset == 'sim-R':
        from SpanPtr_utils.simR_data_utils import prepare_dataset, MultiWozDataset, postprocessing, state_equal, SLOT, OP
    if args.dataset == 'sim-M':
        from SpanPtr_utils.simM_data_utils import prepare_dataset, MultiWozDataset, postprocessing, state_equal, SLOT, OP
    if args.dataset == 'DSTC2':
        from SpanPtr_utils.DSTC2_data_utils import prepare_dataset, MultiWozDataset, postprocessing, state_equal, SLOT, OP
    if args.dataset == 'WOZ2.0':
        from SpanPtr_utils.WOZ_data_utils import prepare_dataset, MultiWozDataset, postprocessing, state_equal, SLOT, OP
    if args.dataset == 'MultiWOZ2.1':
        from SpanPtr_utils.MultiWOZ_data_utils import prepare_dataset, MultiWozDataset, postprocessing, state_equal, OP, make_slot_meta
    ...
    for epoch in range(args.n_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            input_ids, op_ids, gen_ids, input_lens, max_input = batch
            max_value = 2
...
if __name__ == "__main__":
    ...
    # Required parameters
    parser.add_argument("--dataset", default='DSTC2', type=str)
    ...
    if args.dataset == 'sim-R':
        ...
        args.save_dir = 'outputs/SpanPtr/sim-R_outputs'
    elif args.dataset == 'sim-M':
        data_root = 'data/M2M/sim-M'
        args.train_data_path = os.path.join(data_root, 'train.json')
        args.dev_data_path = os.path.join(data_root, 'dev.json')
        args.test_data_path = os.path.join(data_root, 'test.json')
        args.save_dir = 'outputs/SpanPtr/sim-M_outputs'
    elif args.dataset == 'DSTC2':
        data_root = 'data/DSTC2'
        args.train_data_path = os.path.join(data_root, 'train.json')
        args.dev_data_path = os.path.join(data_root, 'dev.json')
        args.test_data_path = os.path.join(data_root, 'test.json')
        args.save_dir = 'outputs/SpanPtr/DSTC2_outputs'
        ...
        args.save_dir = 'outputs/SpanPtr/WOZ_outputs'
        ...
        args.save_dir = 'outputs/SpanPtr/MultiWOZ_outputs'
    else:
